...
- [Phase 3 Production Readiness — Decision Log](architecture/phase3-production-readiness.md) — v0.3.0-beta production hardening: Waves A–F (18 tasks), configurable star threshold, confidence-aware Gate 1/2 composite, adaptive content strategy, weighted confidence, tenacity retry, heuristic detection path-based, persistent assessment cache | Updated: 2026-04-27
...
- [GitHub API Patterns and Constraints](apis/github-api-patterns.md) — REST/GraphQL API usage patterns, smart rate limit handling (30s cap, adaptive throttle Semaphore(3)), pagination, MCP composition | Updated: 2026-04-27
...
- [Phase 4 Deep Assessment Implementation](patterns/phase4-assessment-implementation.md) — Deep Technical Assessment (Layer C) COMPLETE + VERIFIED + PRODUCTION READINESS PLAN v1: NanoGPT provider, repomix packing, 8 dimension prompts, content strategy (adaptive tokens), soft budget controller, heuristic fallback (confidence cap 0.2), tenacity retry, persistent DeepAssessmentResult cache, hard daily limit option, doctor CLI pre-flight checks, 230 assessment tests (1476 total) | Updated: 2026-04-27
- [Phase 5 Scoring & Ranking Implementation](patterns/phase5-scoring-implementation.md) — Scoring & Ranking (Layer D) COMPLETE + STAR-NEUTRAL REDESIGN + FASE 2 AUDIT + PRODUCTION READINESS: ScoringEngine, 12 domain profiles, ConfidenceCalculator (weighted+penalty), Ranker (blake2b deterministic), FeatureStore (SQLite, assessment_results table), ScoreResult.degraded field, per-profile derivation maps (T5.1), custom YAML/TOML profiles (T5.3), 1476 total tests | Updated: 2026-04-27
